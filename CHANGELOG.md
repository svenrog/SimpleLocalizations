...
- A simple path: `<VocabularyResource Include="Strings.resx" />` is a whole declaration. The class name,
  namespace, resource name and key type all default from the file, the package ships a `TextKey` for a
  project that keys one kind of thing, and the generated class carries a `Catalog(TextCultures)` so a
